...
export function App({ adapter = defaultReviewAdapter }: AppProps) {
  ...
  async function correctTerm(correction: TermCorrection) {
    ...
    setSaving(true);
    try {
      const updated = await adapter.correctTerm(review.id, selectedFinding.id, correction);
  ...
  function replaceFinding(updated: ReviewFinding) {
    setReview((current) =>
      current
        ? { ...current, findings: current.findings.map((finding) => (finding.id === updated.id ? updated : finding)) }
        : current,
    );
  }
  ...
  async function requestExport(format: ExportFormat) {
    ...
    try {
      ...
      setNotice({ tone: "success", message: `${result.filename} downloaded.` });
    } catch (error) {
