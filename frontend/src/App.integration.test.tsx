...
describe("BEACON runtime integration", () => {
  it("reloads the complete immutable snapshot after human review, preserving the financial result", async () => {
    ...
    const prepareEmail = vi.spyOn(adapter, "prepareEmail").mockResolvedValue({
      id: "version-2-draft", status: "DRAFT", recipient: "", subject: "Version 2 review", body: "Draft only", attachments: [],
    });
    render(<App adapter={adapter} />);
    await user.click(screen.getByRole("button", { name: "Load synthetic demo" }));
    await user.click(await screen.findByRole("button", { name: "Open LP03 Management fee finding" }));
    ...
    await user.click(screen.getByRole("button", { name: "Open LP04 Management fee finding" }));
  ...
  it.each([
    ...
    expect(screen.getByText(snapshot.sourceNotice!)).toBeInTheDocument();
  });
