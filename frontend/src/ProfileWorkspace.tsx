...
import { isTerminalJob, workspaceAdapter as defaultWorkspaceAdapter } from "./api/workspaceAdapter";
...
export function ProfileWorkspace({ adapter = defaultWorkspaceAdapter, active = true }: ProfileWorkspaceProps) {
  ...
  const [started, setStarted] = useState<StartJobResponse>();
  ...
  useEffect(() => {
    ...
    void adapter.bootstrap().then((next) => {
      if (cancelled) return;
      setBootstrap(next);
      const firstCompatible = next.profiles.find((profile) =>
        next.capabilities?.profiles.some((capability) => capability.profile_id === profile.id),
      );
      setProfileId((current) => current || firstCompatible?.id || next.profiles[0]?.id || "");
    }).catch((error) => {
      if (cancelled) return;
  ...
  async function startReview() {
    if (manifestIssues.length > 0 || !confirmed || busy) return;
      ...
      <div id="profile-workspace-content" tabIndex={-1}>
        {screen === "INTAKE" && (
          <WorkspaceIntake
            ...
            onCaseNameChange={(value) => { setCaseName(value); setConfirmed(false); submissionRef.current = undefined; }}
        ...
        {screen === "PROGRESS" && started && (
          <JobProgress
            started={started}
            job={job}
            profileLabel={bootstrap.profiles.find((profile) => profile.id === started.profile_id)?.label ?? started.profile_id}
            connection={liveConnection}
            pollError={pollError}
          />
        )}
        ...
        {screen === "RESULT" && result && (
          <ProfileReviewDesk
            result={result}
            profileLabel={bootstrap.profiles.find((profile) => profile.id === result.profile_id)?.label ?? result.profile_id}
            connection={liveConnection}
            capabilities={capabilities}
            reviewing={reviewing}
