...
export function WorkspaceIntake({
  ...
  const canStart = selectionReady && issues.length === 0 && !busy && !scanning;
    ...
    <div className="intake-page">
      <section className="intake-intro" aria-labelledby="intake-title">
        ...
        <p className="intake-lede">
          Add a bank-statement PDF or inspect a complete folder. Choose <strong>Start review</strong>
          when the selected files are ready. Selecting files never starts processing automatically.
        </p>
        ...
        {connection.state !== "CONNECTED" && (
          <button className="button button-secondary" type="button" disabled={checkingConnection || busy} onClick={onRetryConnection}>
            {checkingConnection ? "Checking connection…" : "Retry connection"}
          </button>
        )}
        ...
        {entries.length > 0 && (
          <div className="inventory-area">
            ...
            <div className="manifest-confirmation">
              <p>{selected.length} {selected.length === 1 ? "file" : "files"} selected for this review. Only selected inputs will be uploaded.</p>
