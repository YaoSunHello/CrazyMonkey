...
print("Columns:", t_acc.columns)
for acc in t_acc.values('Account Number'):
    print(f"  {acc} -> {t_acc.find('Account Number', acc)}")

print("\n=== PROJECT CODES ===")
t_proj = kit.table('project_codes')
print("Project Code count:", len(t_proj.values('Project Code')))
print("Sample Project Code:", t_proj.values('Project Code')[:10])
print("Sample New Project Code:", t_proj.values('New Project Code')[:10])

# Test project code lookups on tokens from narratives
proj_candidates = ['RI00034', 'LO-DK00629', '499', '032424373', '524830540', '89732SV16BGK', '55633BC44JQ0', '08334GU5571N', '44008EZ203K5']
for pc in proj_candidates:
    res = kit.lookup(pc, [('project_codes', 'Project Code'), ('project_codes', 'New Project Code')])
    cands = kit.candidates(pc, [('project_codes', 'Project Code'), ('project_codes', 'New Project Code')])
    print(f"Project lookup '{pc}': {res} (candidates: {cands})")
...
parties = [
    'ULLA B. HILLEBRANDT CONSULTING',
    'NIP LIT',
    'NORDVIK INFRASTRUCTURE ADVANCED',
    'NISC APS',
    'RAQ MARBECK COMMUNICATIONS',
    'LU HBEU',
    'HBEU'
]

for p in parties:
    res = kit.lookup(p, pools)
    cands = kit.candidates(p, pools)
    print(f"\nParty '{p}':")
    print(f"  exact: {res}")
    print(f"  candidates: {cands}")

# Also check candidate substrings across all narratives
print("\n=== CHECKING ALL NARRATIVE CHUNKS ===")
for i, row in enumerate(kit.rows()):
    narr = row['narrative']
    print(f"\nRow {i}: trn_type={row['trn_type']} narr='{narr}'")
    # Split by comma or slash or whitespace to test lookups
    parts = [p.strip() for p in narr.replace('/', ',').split(',') if p.strip()]
    for part in parts:
        res = kit.lookup(part, pools)
        if res:
            print(f"  MATCH '{part}': {res}")
        else:
            # check candidates if not obviously an ID or number
            if not any(c.isdigit() for c in part) and len(part) > 3:
                cands = kit.candidates(part, pools)
                print(f"  No match for '{part}', candidates: {cands}")
```
