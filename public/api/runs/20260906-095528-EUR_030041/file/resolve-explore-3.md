...
print("=== INSPECTION SCRIPT ===")

# Allowed pools for counterparty:
# legal_entities:Legal Entity
# related_parties:Related Party
# vendors:Vendor
# investors:Investor
...
# Let's inspect all tables and their sizes
for t_name in kit.tables():
    t = kit.table(t_name)
    print(f"Table {t_name}: {t.columns}")

print("\n--- Search for Cinnabar, Garrfield, Draconis, Ranfjord, BQVRFRPP, Nordvik ---")
for kw in ['Cinnabar', 'Garrfield', 'Garfield', 'Draconis', 'Ranfjord', 'BQVRFRPP', 'Nordvik', 'FR62', 'Chalworth', 'ISIN', 'TRANCHE']:
    print(f"\nKeyword: {kw}")
    for tbl_name, col in cp_pools:
        t = kit.table(tbl_name)
        vals = [v for v in t.values(col) if kw.lower() in v.lower()]
        if vals:
            print(f"  {tbl_name}.{col}: {vals[:5]}")

print("\n--- Search for Project Codes ---")
for kw in ['Ranfjord', 'Draconis', 'Cinnabar', 'B1', 'Tranche']:
    t = kit.table('project_codes')
    for col in t.columns:
        vals = [v for v in t.values(col) if kw.lower() in v.lower()]
        if vals:
            print(f"  project_codes.{col}: {vals[:5]}")

print("\n--- Account Map ---")
t_acc = kit.table('account_map')
for row in t_acc.find('Account Number', '240-222731-030'):
    print("  Account 240-222731-030:", row)

print("\n--- Rows Detailed Analysis ---")
rows = kit.rows()
for i, r in enumerate(rows, 1):
    narr = r['narrative']
    print(f"\nRow {i}: trn_type={r.get('trn_type')} debit={r.get('debit')} credit={r.get('credit')}")
    print(f"  narrative: {narr}")
    
    # Try looking up substrings
    words = narr.replace(',', ' ').replace('/', ' ').split()
    # Check candidates for parts of narrative
    # Let's search candidates in cp_pools
    cand_results = []
    # Test chunks of length 1 to 6
    raw_tokens = narr.split()
    found_cp = []
    for l in range(min(6, len(raw_tokens)), 0, -1):
        for start in range(len(raw_tokens) - l + 1):
            sub = " ".join(raw_tokens[start:start+l]).strip(" ,./")
            if len(sub) < 3:
                continue
            res = kit.lookup(sub, cp_pools)
            if res:
                found_cp.append((sub, res))
    print(f"  Found exact CP: {found_cp}")
    
    # Let's also check candidates for potential words
    for w in ['Cinnabar', 'NIP CINNABAR', 'GARRFIELD', 'BQVRFRPP', 'Ranfjord', 'Draconis']:
        if w.lower() in narr.lower():
            cands = kit.candidates(w, cp_pools, limit=3)
            print(f"  Candidates for '{w}': {cands}")
