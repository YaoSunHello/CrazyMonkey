...
print("=== TABLES ===")
for name in kit.tables():
    t = kit.table(name)
    print(f"Table: {name}, columns: {t.columns}")
...
for tbl, col in pools_to_check:
    if tbl in kit.tables():
        t = kit.table(tbl)
        vals = t.values(col) if col in t.columns else []
        print(f"{tbl}:{col} -> count: {len(vals)}, samples: {vals[:5]}")

print("Done exploration round 1.")
```
