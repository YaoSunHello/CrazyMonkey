...
print("=== ACCOUNT MAP ===")
t_acct = kit.table("account_map")
for row in t_acct.values("Account Number"):
    print(row, "->", t_acct.find("Account Number", row))
...
for tname in [
    ...
    for col in t.columns:
        vals = t.values(col)
        print(f"  {col} ({len(vals)} items): {vals[:10]}")
        if len(vals) > 10:
            print(f"    ... {vals[10:25]}")

print("\n=== ROW 15 ===")
rows = kit.rows()
print("Row 15:", rows[15])

print("\n=== ALL NARRATIVES & BANK REFERENCES ===")
for i, r in enumerate(rows):
    print(f"Row {i}:")
    print(f"  bank_ref: {r.get('bank_reference')}")
    print(f"  narrative: {r.get('narrative')}")
    print(f"  credit: {r.get('credit')}, debit: {r.get('debit')}")
```
