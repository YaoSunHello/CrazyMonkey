...
for row_idx in range(len(t_acc.values('Account Number'))):
    acc = t_acc.values('Account Number')[row_idx]
    bank_acc = t_acc.values('Bank Account')[row_idx]
    print(f"  {acc} -> {bank_acc}")

print("\n=== SEARCH ALL TABLES FOR NIP, NORDVIK, LIT, FREJA, MOERCH ===")
search_terms = ['FREJA', 'MOERCH', 'MØRCH', 'MORCH', 'LIT', 'NIP', 'NORDVIK', 'CALDER']
        ...
        for val in vals:
            ...
            for term in search_terms:
                if term in val_upper:
                    print(f"[{tbl_name}][{col}] ({term}): {val}")

print("\n=== LOOKUP ALL SUBSTRINGS OF NARRATIVES ===")
pools_cp = [
    ('legal_entities', 'Legal Entity'),
    ('related_parties', 'Related Party'),
    ('vendors', 'Vendor'),
...
for idx, r in enumerate(kit.rows()):
    print(f"\n--- ROW {idx} ---")
    print(f"narrative: {r['narrative']}")
    print(f"trn_type: {r['trn_type']}, debit: {r.get('debit')}, credit: {r.get('credit')}")
    narrative = r['narrative']
    words = narrative.replace(',', ' ').split()
    # Try all contiguous spans of words (length 1 to 5)
    for l in range(1, 6):
        for i in range(len(words) - l + 1):
            span = " ".join(words[i:i+l])
            # Strip slashes or punctuation
            clean_span = span.strip('/.,-')
            if clean_span:
                res_cp = kit.lookup(clean_span, pools_cp)
                if res_cp:
                    print(f"  CP MATCH for '{clean_span}' (from '{span}'): {res_cp}")
                res_pr = kit.lookup(clean_span, pools_proj)
                if res_pr:
                    print(f"  PROJ MATCH for '{clean_span}' (from '{span}'): {res_pr}")

print("\n=== CANDIDATES FOR ROW STRINGS ===")
for test_str in ['NIP LIT', 'LIT', 'FREJA MOERCH', 'NORDVIK INFRASTRUCTURE PARTNER', 'NORDVIK INFRASTRUCTURE PARTNERS', 'NIP']:
    print(f"\nCandidates for '{test_str}':")
    c_cp = kit.candidates(test_str, pools_cp)
    for c in c_cp:
        print(f"  CP: {c}")
    c_pr = kit.candidates(test_str, pools_proj)
    for c in c_pr:
        print(f"  PR: {c}")

print("Done exploration pass 3.")
```
