...
for term in search_terms:
    print(f"\nSearching for '{term}':")
    for tbl_name, col in pools:
        t = kit.table(tbl_name)
        vals = t.values(col)
        matches = [v for v in vals if v and term.lower() in v.lower()]
        if matches:
            print(f"  [{tbl_name}:{col}] found {len(matches)}: {matches[:5]}")

print("\n=== TESTING CANDIDATES / LOOKUPS ON NARRATIVES ===")
for i, r in enumerate(rows, 1):
    narr = r["narrative"]
    print(f"\nRow {i}: {narr}")
    # test variants / lookup
    # look for project code
    # Usually project word is after 'PROJECT '
    if "PROJECT " in narr:
        proj_word = narr.split("PROJECT ")[1].split()[0].rstrip(".")
        res = kit.lookup(
            proj_word,
            [
                ("project_codes", "Project Code"),
        ...
        print(f"  Project lookup for '{proj_word}': {res}")
    # test counterparty lookups
    # Let's test a few plausible substrings
    # E.g. leading field before comma
    leading = narr.split(",")[0].strip()
    res_lead = kit.lookup(
        leading,
        [
            ("legal_entities", "Legal Entity"),
    ...
    print(f"  Leading '{leading}': {res_lead}")
    if not res_lead:
        cands = kit.candidates(
            leading,
            [
                ("legal_entities", "Legal Entity"),
                ("related_parties", "Related Party"),
        ...
        print(f"    candidates for '{leading}': {cands}")

print("\nFinished exploration 2")
```
