...
print("=== CHECKING MATCHES FOR PARTIES AND PROJECTS ===")

# Allowed reference pools
party_pools = [
    ('legal_entities', 'Legal Entity'),
    ('related_parties', 'Related Party'),
    ('vendors', 'Vendor'),
...
# Let's inspect all tables in detail
for t_name in kit.tables():
    t = kit.table(t_name)
    print(f"\n--- Table: {t_name} ---")
    for col in t.columns:
        vals = t.values(col)
        print(f"  {col}: {len(vals)} entries")

# Search terms to explore
search_terms = [
    'KALVIK', 'FENWICK', 'AZURITE', 'WILLOWBANK', 'NORDVIK', 'NI V', 'SCSP', 'CN SCSP',
    'INFRASTRUCTURE', 'INTERNAL TRANSFER', 'PARTNER'
]

print("\n=== SEARCHING IN REFERENCE TABLES ===")
for term in search_terms:
    print(f"\nSearch for '{term}':")
    for tbl, col in party_pools + proj_pools:
        t = kit.table(tbl)
        matches = [v for v in t.values(col) if term.lower() in v.lower()]
        if matches:
            print(f"  [{tbl}:{col}] ({len(matches)}): {matches[:10]}")

# Let's check each row's narrative and try spans/lookups
rows = kit.rows()
print(f"\n=== ANALYZING 16 ROWS ===")
for i, r in enumerate(rows, 1):
    narr = r['narrative']
    print(f"\nRow {i}: narrative = {repr(narr)}")
    
    # Try project code keywords
    # Often after "PROJECT "
    proj_code = None
    if "PROJECT " in narr:
        after_proj = narr.split("PROJECT ")[1]
        # single token after PROJECT
        proj_token = after_proj.strip().split()[0].rstrip('.,;')
        print(f"  Found project token: {repr(proj_token)}")
        proj_res = kit.lookup(proj_token, proj_pools)
        print(f"  Lookup({proj_token}): {proj_res}")
        if not proj_res:
            cands = kit.candidates(proj_token, proj_pools)
            print(f"  Candidates for project: {cands}")

    # Let's test potential counterparty candidates in narrative
    # Test kit.lookup on various candidate substrings
    words = narr.replace(',', ' , ').split()
    found_matches = []
    # Test n-grams up to length 7
    for n in range(1, 8):
        for start in range(len(words) - n + 1):
            span_words = words[start:start+n]
            # reconstruct span
            span_text = " ".join(span_words).replace(" ,", ",")
            # test lookup
            res = kit.lookup(span_text, party_pools)
            if res:
                found_matches.append((span_text, res))
    
    # deduplicate found_matches by matched value
    unique_matches = {}
    for span_text, res in found_matches:
        key = (res['table'], res['matched_name'])
        if key not in unique_matches:
            unique_matches[key] = (span_text, res)
    
    print(f"  Exact party matches found: {list(unique_matches.values())}")
```
