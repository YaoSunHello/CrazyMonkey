...
for idx, r in enumerate(rows):
    ...
    # Let's test kit.lookup on various tokens/spans
    narr = r.get("narrative", "")

    # Check project code
    # Usually after PROJECT or similar keyword, or single tokens
    # Let's search for matches with proj_pools
    words = narr.replace(",", " ").replace("(", " ").replace(")", " ").split()
    found_proj = []
    for w in words:
        res = kit.lookup(w, proj_pools)
        if res:
            found_proj.append((w, res))
    print(f"Found project codes: {found_proj}")

    # Let's test finding counterparties
    # Try n-grams of words
    found_parties = []
    clean_words = narr.replace(",", " , ").split()
    for n in range(1, 8):
        for i in range(len(words) - n + 1):
            phrase = " ".join(words[i : i + n])
            res = kit.lookup(phrase, pools)
            if res:
                found_parties.append((phrase, res))
    # Deduplicate by matched_name
    unique_parties = {}
    for p, res in found_parties:
        unique_parties[res["matched_name"]] = (p, res)
    print(f"Found counterparties: {list(unique_parties.values())}")

print("\n=== LOOKUP CHECK FOR SPECIFIC STRINGS ===")
test_strings = [
    "NI ABF I SCSP",
    "NI ABF I",
    "NI ABF II SCSP",
    ...
    "NI ABF II QFPF BLOC. SCSP",
    "NORDVIK INFRASTRUCTURE ADVANCED",
    "NIP PLATFORM SOLUTIONS APS",
    "TRENTBECK AUDIT LUXEMBOURG",
    "CEPHALUS BIOGAS 001 LTD",
    "CEPHALUS",
]
for s in test_strings:
    res = kit.lookup(s, pools)
    print(f"Lookup '{s}': {res}")
    if not res:
        cands = kit.candidates(s, pools, limit=3)
        print(f"  Candidates for '{s}': {cands}")

print("\nSummary: done exploring rows and tables")
```
