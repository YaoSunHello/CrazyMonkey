...
def draft_subject(snapshot) -> str:  # type: ignore[no-untyped-def]
    ...
    return (
        f"YLookup NAV Review - {snapshot.reporting_period} - "
...
def draft_body(snapshot) -> str:  # type: ignore[no-untyped-def]
    ...
    return (
        "Hi,\n\n"
        f"YLookup completed the management-fee review for {snapshot.reporting_period}.\n\n"
...
def write_eml_draft(
    ...
    message["X-YLookup-Status"] = "DRAFT_NOT_SENT"
    message["X-YLookup-Run-ID"] = frozen.snapshot.run_id
    message["X-YLookup-Review-Version"] = str(frozen.snapshot.version)
    message["X-YLookup-Snapshot-SHA256"] = frozen.snapshot_sha256
