...
class Calculation(RelayModel):
    ...
    annual_rate_fraction: float | None = Field(default=None, ge=0)
    period_factor: float | None = Field(default=None, ge=0)
...
class InvestorTerm(RelayModel):
    investor_id: str
    term: str
    default_rate_fraction: float | None = Field(default=None, ge=0)
    override_rate_fraction: float | None = Field(default=None, ge=0)
    applicable_rate_fraction: float | None = Field(default=None, ge=0)
...
class Finding(RelayModel):
    ...
    investor_id: str
    check_type: str
    administrator_value: float | None = None
    expected_value: float | None = None
    difference: float | None = None
    ...
    def validate_cross_references(self) -> "OutputSnapshotView":
        ...
        for event in self.audit_trail:
            if event.run_version != self.version:
                raise ValueError("audit event run_version must match the snapshot version")
