...
from .models import OutputSnapshotView, exact_decimal
        ...
        calculations.append(
            {
                ...
                "fee_base": _optional_decimal(calculation.fee_base),
                "annual_rate_fraction": _optional_decimal(calculation.annual_rate),
                "period_factor": _optional_decimal(calculation.period_factor),
                "expected_value": _optional_decimal(calculation.expected_amount),
                "reported_value": _optional_decimal(calculation.reported_amount),
                "difference": _optional_decimal(calculation.difference),
        ...
        terms.append(
            {
                ...
                "term": "Management fee",
                "default_rate_fraction": _optional_decimal(rule.default_annual_rate),
                "override_rate_fraction": _optional_decimal(rule.candidate_override_rate),
                "applicable_rate_fraction": _optional_decimal(rule.applicable_annual_rate),
                "effective_from": rule.effective_from.isoformat() if rule.effective_from else None,
                "effective_to": rule.effective_to.isoformat() if rule.effective_to else None,
                "fee_base": _optional_decimal(rule.fee_base),
        ...
        "findings": [
            {
                ...
                "administrator_value": _optional_decimal(finding.reported_value),
                "expected_value": _optional_decimal(finding.expected_value),
                "difference": _optional_decimal(finding.difference),
                "difference_convention": "ATLAS_REPORTED_MINUS_EXPECTED",
                "variance_direction": _variance_direction(
                    _optional_decimal(finding.reported_value),
                    _optional_decimal(finding.expected_value),
                ),
...
def _validate_atlas_references(atlas: AtlasReviewSnapshot) -> None:
    ...
    for event in atlas.audit_trail:
        if event.run_id != atlas.run_id:
            raise SnapshotContractError("Atlas audit event run_id does not match its snapshot")
        if event.run_version > atlas.version:
            raise SnapshotContractError("Atlas audit event version exceeds its snapshot")
...
def _money(value: Any) -> tuple[Decimal | None, str | None]:
    if not isinstance(value, Mapping):
        return None, None
    try:
        amount = exact_decimal(value["amount"])
...
def _parse_calculation_inputs(inputs: Any) -> dict[str, Decimal | None]:
    ...
    for item in inputs:
        ...
        if "base" in label:
            ...
            result["annual_rate_fraction"] = value / Decimal("100") if value is not None else None
        elif "factor" in label or "quarter" in label:
            result["period_factor"] = value
    return result


def _find_rates(item: Mapping[str, Any]) -> dict[str, Decimal | None]:
    default_rate = None
    override_rate = None
    for evidence in item.get("evidence", []):
...
def _variance_direction(admin: Decimal | None, expected: Decimal | None) -> str:
