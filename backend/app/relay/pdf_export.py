...
def write_pdf_report(path: Path, frozen: FrozenSnapshot, generated_at: datetime) -> None:
    ...
    story.extend(
        [
            Paragraph("YLOOKUP NAV REVIEW", styles["title"]),
            Paragraph(
    ...
    def decorate_page(canvas, doc) -> None:  # type: ignore[no-untyped-def]
        canvas.saveState()
        canvas.setTitle("YLookup NAV Review")
        canvas.setAuthor("YLookup RELAY")
        canvas.setSubject(
...
def validate_pdf(path: Path) -> None:
    ...
    required = ("YLOOKUP NAV REVIEW", "Management-fee checks only", "LIMITATIONS")
