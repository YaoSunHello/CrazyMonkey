...
def download_beacon_compatible_export(
    ...
    """Bridge Beacon's download route to an explicitly displayed frozen version."""

    return _artifact_response(run_id, version, artifact_type, None)
...
def prepare_beacon_compatible_email_draft(
    run_id: str, version: int = Query(..., ge=1),
) -> dict[str, Any]:
    """Prepare only the explicitly displayed immutable review version."""
