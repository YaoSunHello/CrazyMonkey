...
from .utils import decimal_text, iso_z, mode_label, spreadsheet_literal, spreadsheet_number
...
def write_xlsx_report(
    ...
    workbook.set_custom_property("CrazyMonkey Run ID", snapshot.run_id)
    workbook.set_custom_property("CrazyMonkey Review Version", str(snapshot.version))
    workbook.set_custom_property("CrazyMonkey Snapshot SHA256", frozen.snapshot_sha256)
    workbook.set_custom_property("CrazyMonkey Generated At", iso_z(generated_at))
    workbook.set_custom_property("CrazyMonkey Mode", snapshot.mode.value)
...
def _add_sheet(workbook, name: str, tab_color: str | None = None):  # type: ignore[no-untyped-def]
    ...
    worksheet.set_header("&LCrazyMonkey NAV Review&R&P of &N")
    return worksheet
...
def _write_summary(workbook, formats, snapshot, frozen, generated_at):  # type: ignore[no-untyped-def]
    ...
    row = _sheet_header(sheet, formats, "CrazyMonkey NAV Review", snapshot, frozen, generated_at, 8)
...
def _write_calculations(workbook, formats, snapshot, frozen, generated_at):  # type: ignore[no-untyped-def]
    ...
    for index, calculation in enumerate(snapshot.calculations, start=row + 1):
        ...
        formula = f"=ROUND(B{excel_row}*C{excel_row}*D{excel_row},2)"
        formula_values = (
            spreadsheet_number(calculation.fee_base),
            spreadsheet_number(calculation.annual_rate_fraction),
            spreadsheet_number(calculation.period_factor),
            spreadsheet_number(calculation.expected_value),
        )
        if all(
            value is not None
            for value in formula_values
        ):
            sheet.write_formula(index, 4, formula, formats["currency"], formula_values[-1])
            ...
            sheet.write_string(
                index,
                4,
                "Server-verified Decimal result; spreadsheet formula omitted to prevent precision loss",
                formats["wrap"],
            )
            sheet.write_string(index, 5, formula, formats["literal_formula"])
...
def _write_number_or_blank(sheet, row: int, column: int, value: Any, cell_format: Any) -> None:  # type: ignore[no-untyped-def]
    if value is None:
        sheet.write_blank(row, column, None, cell_format)
    else:
        numeric = spreadsheet_number(value)
        if numeric is None:
            sheet.write_string(row, column, decimal_text(value), cell_format)
        else:
            sheet.write_number(row, column, numeric, cell_format)
