...
def _ui_eligible(profile) -> bool:
    """The same runnable subset is advertised and accepted by this bridge."""
    source_kind = str(profile.inputs.get("documents", {}).get("kind", "")).casefold()
    return bool(profile.passes) and source_kind == "pdf"
...
def create_job(raw_manifest: str, uploads: list[UploadFile], idempotency_key: str | None) -> tuple[Job, bool]:
    ...
    if not _ui_eligible(profile):
        raise _error(422, "PROFILE_NOT_SUPPORTED", "this profile is not supported by the UI bridge")
...
def _document_citation(item: StoredInput) -> dict[str, Any]:
    return {
        "source_id": item.source_id,
        "filename": item.filename,
        "atlas_document_id": item.atlas["document_id"] if item.atlas else None,
...
def _normalize_input(job: Job, item: StoredInput) -> None:
    """Run the shared ATLAS intake over the exact bytes stored for this source."""
    storage_key = f"{job.job_id}/{item.source_id}/{item.filename}"
    # These bank-statement inputs are supporting documents in ATLAS, not NAV
    # workbooks or investor registers. SOURCE/REFERENCE remains the UI purpose.
    metadata = {
        "document_id": None,
        "document_hash": item.sha256,
        "role": DocumentRole.SUPPORTING.value,
    ...
    try:
        ...
        if normalized.document.document_hash != item.sha256:
            raise IngestionError("SOURCE_CHANGED", "Stored source bytes changed after upload")
        metadata.update(
            normalized.document.model_dump(mode="json"),
            evidence_count=len(normalized.evidence),
        )
    except IngestionError as exc:
        metadata["warnings"] = [exc.message]
        raise
    finally:
        with job.lock:
            item.atlas = metadata
...
def _input_error(exc: Exception) -> str:
    ...
    if isinstance(exc, UnsupportedStatementError):
        return f"UNSUPPORTED_STATEMENT: {exc}"
...
def _source_result(job: Job, item: StoredInput, profile) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    for check in checks:
        ...
        check_data = {
            ...
            "source_citation": _document_citation(item),
            "review_status": "UNREVIEWED",
            "updated_at": None,
        }
        finding = {
