...
(function () {
  ...
  function paintTop() {
    ...
    $("tabs").innerHTML = [
      ...
      // A published snapshot cannot start one, so the tab says what it is for.
      ["new", window.CM_STATIC ? "Profiles" : "New run", ""],
    ].map(([id, label, count]) =>
      ...
      const listRuns = () => {
        ...
        $("profruns").querySelectorAll("[data-run]").forEach((link) => {
          link.onclick = (event) => {
            ...
            Promise.resolve(openRun(link.dataset.run)).then(() => show("replay"));
          };
        });
      };
