...
print("=== EXPLORING ROWS AND MATCHES ===")

# Pools for counterparty:
# legal_entities:Legal Entity
# related_parties:Related Party
# vendors:Vendor
# investors:Investor
...
for row in kit.rows():
    print(f"\nRow {row['narrative']}:")
    print(f"  trn_type: {row['trn_type']}")
    print(f"  account: {row['account_number']}")
    
    # Try looking up various substrings
    narr = row['narrative']
    parts = [p.strip() for p in narr.split(',')]
    print(f"  parts: {parts}")
    
    for p in parts:
        res = kit.lookup(p, cp_pools)
        if res:
            print(f"    CP match for '{p}': {res}")
        else:
            cands = kit.candidates(p, cp_pools, limit=3)
            if cands:
                print(f"    CP cands for '{p}': {cands}")
                
        pres = kit.lookup(p, proj_pools)
        if pres:
            print(f"    Proj match for '{p}': {pres}")
        else:
            pcands = kit.candidates(p, proj_pools, limit=3)
            if pcands:
                print(f"    Proj cands for '{p}': {pcands}")

print("\n=== SEARCHING FOR SPECIFIC TOKENS ===")
tokens = ['NIP LIT', 'LIT', 'FREJA MOERCH', 'MOERCH', 'NORDVIK INFRASTRUCTURE PARTNER', 'NORDVIK']
for t in tokens:
    res = kit.lookup(t, cp_pools)
    print(f"Lookup '{t}': {res}")
    cands = kit.candidates(t, cp_pools, limit=3)
    print(f"Candidates '{t}': {cands}")
    pres = kit.lookup(t, proj_pools)
    print(f"Lookup proj '{t}': {pres}")
    pcands = kit.candidates(t, proj_pools, limit=3)
    print(f"Candidates proj '{t}': {pcands}")

# Also search directly in tables
print("\n=== SEARCH IN TABLES DIRECTLY ===")
for tbl_name in ['vendors', 'related_parties', 'legal_entities', 'investors', 'deals_positions', 'project_codes']:
    tbl = kit.table(tbl_name)
    for col in tbl.columns:
        vals = tbl.values(col)
        for v in vals:
            v_str = str(v)
            if any(k in v_str.upper() for k in ['FREJA', 'MOERCH', 'LIT', 'NORDVIK INFRASTRUCTURE']):
                print(f"Found in {tbl_name}[{col}]: {v}")

print("Exploration pass 2 completed.")
```
