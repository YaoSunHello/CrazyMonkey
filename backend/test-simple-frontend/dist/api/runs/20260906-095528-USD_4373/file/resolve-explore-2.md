...
# Inspect account map and see if 240-644826-130 is present
t_acc = kit.table('account_map')
print("account_map accounts:", t_acc.values('Account Number'))
print("account_map bank accounts:", t_acc.values('Bank Account'))
for r in t_acc.find('Account Number', '240-644826-130'):
    print("Found account in account_map:", r)
...
for i, r in enumerate(rows):
    print(f"--- Row {i} ---")
    print(f"  trn_type: {r.get('trn_type')}")
    print(f"  narrative: {r.get('narrative')}")
    print(f"  debit: {r.get('debit')} | credit: {r.get('credit')}")
    print(f"  account: {r.get('account_number')}")

# Also let's inspect legal_entities and related_parties related to NI GMF II, Nordvik, Calder, etc.
t_le = kit.table('legal_entities')
le_vals = t_le.values('Legal Entity')
print("\nSample legal entities with GMF or Nordvik:")
for v in le_vals:
    if any(w in v.lower() for w in ['gmf', 'nordvik', 'cooperatief', 'scsp', 'growth']):
        print("  LE:", v)

t_rp = kit.table('related_parties')
rp_vals = t_rp.values('Related Party')
print("\nSample related parties with GMF or Nordvik:")
for v in rp_vals:
    if any(w in v.lower() for w in ['gmf', 'nordvik', 'cooperatief', 'scsp', 'growth']):
        print("  RP:", v)
...
print("\nProject codes count:", len(t_pc.values('Project Code')))
```
