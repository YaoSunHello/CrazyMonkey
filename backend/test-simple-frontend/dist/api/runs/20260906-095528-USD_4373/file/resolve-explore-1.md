...
for t_name in tables:
    t = kit.table(t_name)
    print(f"\nTable '{t_name}': columns = {t.columns}")
    # print first few rows of each column
    for col in t.columns:
        vals = list(t.values(col))
        print(f"  Col '{col}' ({len(vals)} entries): sample -> {vals[:5]}")
...
print("Exploration finished: inspected tables and rows.")
```
