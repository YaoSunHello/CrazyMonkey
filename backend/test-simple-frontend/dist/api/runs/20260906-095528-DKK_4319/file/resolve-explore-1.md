...
for t_name in kit.tables():
    t = kit.table(t_name)
    print(f"Table: {t_name}, Columns: {t.columns}")
...
print("\n=== SAMPLE TABLE VALUES ===")
for t_name in ['legal_entities', 'related_parties', 'vendors', 'investors', 'deals_positions', 'project_codes']:
    if t_name in kit.tables():
        t = kit.table(t_name)
        print(f"\n--- {t_name} ---")
        for col in t.columns:
            vals = t.values(col)
            print(f"  Column {col} ({len(vals)} values): sample: {vals[:10]}")
