...
for i in range(10, len(rows)):
    r = rows[i]
    print(f"\n--- ROW {i} ---")
    print(f"trn_type: {r.get('trn_type')} | debit: {r.get('debit')} | credit: {r.get('credit')}")
...
print("\nAccount map:")
for col in t_acc.columns:
    print(col, t_acc.values(col))

# Let's inspect rows 0 to 15 narratives and candidate lookups
pools = [
    ('legal_entities', 'Legal Entity'),
    ('related_parties', 'Related Party'),
    ('vendors', 'Vendor'),
...
for i, r in enumerate(rows):
    ...
    words = nar.replace(',', ' ').replace('(', ' ').replace(')', ' ').replace(':', ' ').split()
    found_p = []
    for w in words:
        res = kit.lookup(w, proj_pools)
        if res:
            found_p.append((w, res))
    print(f"  Project words found: {found_p}")
    
    # Check candidate phrases for counterparties
    # Let's test spans of length 1 to 6 words
    clean_nar = nar.replace(',', ', ')
    # Also variants
    vars = kit.variants(nar)
    found_cp = []
    for v in vars:
        tokens = v.split()
        for l in range(1, min(7, len(tokens) + 1)):
            for start in range(len(tokens) - l + 1):
                ...
                if len(p_clean) < 3:
                    continue
                res = kit.lookup(p_clean, pools)
                if res and (p_clean, res['matched_name'], res['table']) not in [(x[0], x[1]['matched_name'], x[1]['table']) for x in found_cp]:
                    found_cp.append((p_clean, res))
    print(f"  CP found: {[(c[0], c[1]['matched_name'], c[1]['table']) for c in found_cp]}")

    # Also check candidates for parts of narrative if none found
    if not found_cp:
        print("  No CP found via exact lookup. Checking candidates for words/phrases...")
        for tok in tokens:
            if len(tok) > 4:
                cands = kit.candidates(tok, pools, limit=3)
                if cands:
                    print(f"    candidates for {tok}: {cands}")
