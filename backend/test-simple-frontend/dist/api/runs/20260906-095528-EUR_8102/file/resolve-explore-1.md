...
for name in tables:
    t = kit.table(name)
    print(f"\n--- Table: {name} (columns: {t.columns}) ---")
    for col in t.columns:
        vals = t.values(col)
        print(f"  {col} ({len(vals)} values): {vals[:5]}")
...
print("\n=== TESTING LOOKUPS & CANDIDATES ON NARRATIVES ===")
# Let's inspect the narratives and see what words appear in them
counterparty_pools = [
    ('legal_entities', 'Legal Entity'),
    ('related_parties', 'Related Party'),
    ('vendors', 'Vendor'),
...
# Filter pools to valid ones that actually exist
valid_cp_pools = []
for tbl, col in counterparty_pools:
    if tbl in tables:
        t = kit.table(tbl)
        if col in t.columns:
            valid_cp_pools.append((tbl, col))
        else:
            print(f"Column {col} not in table {tbl}, available: {t.columns}")
    else:
        print(f"Table {tbl} not in tables")

print("Valid CP pools:", valid_cp_pools)

for i, r in enumerate(rows):
    nar = r.get('narrative', '')
    print(f"\n--- Row {i} narrative: {repr(nar)} ---")
    words = nar.split()
    # Try n-grams
    matches = []
    for n in range(1, min(len(words) + 1, 8)):
        for start in range(len(words) - n + 1):
            sub = " ".join(words[start:start+n])
            # test lookup
            try:
                res = kit.lookup(sub, valid_cp_pools)
                if res:
                    matches.append((sub, res))
            except Exception as e:
                pass
    if matches:
        print(f"  Found exact matches: {matches}")
    else:
        print("  No n-gram exact match found")
