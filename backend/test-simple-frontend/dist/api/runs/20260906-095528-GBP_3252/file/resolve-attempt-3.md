...
- Caution belongs in the **matching**, not in the reading. Two different jobs:
  ...
  CANNOT_VERIFY.
- A `counterparty_raw` must be a literal substring of that row's own narrative.
  Copy it out rather than reconstructing it, and if the narrative names nobody,
  set it to null with status CANNOT_VERIFY.
...
def main():
    ...
    for idx, r in enumerate(rows, start=1):
        ...
        # Case D: Fenwick HoldCo (sibling numeral IV vs V check)
        elif "FENWICK" in narrative and (
            "HOLDCO" in narrative or "LOAN:" in narrative
        ):
...
- counterparty_raw_provenance: 8/9 counterparty_raw values appear in their own narrative
    row 13: 'NI V FENWICK HOLDCO LTD' is not in this row's narrative
