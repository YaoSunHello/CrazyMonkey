...
def test_generated_source_snapshot_adapts_to_one_valid_output_snapshot(
...
def test_atlas_audit_history_accepts_prior_versions_and_binds_every_event_to_run(
    fixture_payload: dict[str, Any],
) -> None:
    ...
    payload["audit_trail"] = [
        {
            ...
            "action": "REVIEWED" if version == 2 else "NEEDS_FOLLOW_UP",
            "reviewer_label": "Reviewer",
            "timestamp": f"2026-09-05T12:0{version}:00Z",
            "note": f"Review event {version}",
            "previous_review_state": "UNREVIEWED" if version == 2 else "REVIEWED",
            "new_review_state": "REVIEWED" if version == 2 else "NEEDS_FOLLOW_UP",
            "affected_artifact_ids": [],
        }
        for version in (2, 3)
    ]

    adapted = adapt_review_snapshot(payload)

    assert [event.run_version for event in adapted.audit_trail] == [2, 3]
    assert [event.run_id for event in adapted.audit_trail] == [payload["run_id"]] * 2

    wrong_run = copy.deepcopy(payload)
    wrong_run["audit_trail"][0]["run_id"] = "different-run"
    with pytest.raises(SnapshotContractError, match="run_id"):
        adapt_review_snapshot(wrong_run)

    future_event = copy.deepcopy(payload)
    future_event["audit_trail"][0]["run_version"] = 4
    with pytest.raises(SnapshotContractError, match="version"):
        adapt_review_snapshot(future_event)
