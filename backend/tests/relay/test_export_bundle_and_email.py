...
def test_one_frozen_snapshot_generates_complete_idempotent_bundle(
    ...
    assert draft["X-YLookup-Status"] == "DRAFT_NOT_SENT"
    assert draft["X-YLookup-Run-ID"] == first.run_id
    assert draft["X-YLookup-Review-Version"] == str(first.version)
    assert str(draft["X-YLookup-Snapshot-SHA256"]).strip() == first.snapshot_sha256
