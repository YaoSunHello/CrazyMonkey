...
class AtlasIngestionTests(unittest.TestCase):
    ...
    def test_expected_answers_remain_test_only_and_are_internally_consistent(self) -> None:
        expected_path = Path(__file__).parent / "atlas" / "expected" / "synthetic_expected.json"
        expected = json.loads(expected_path.read_text(encoding="utf-8"))
        statuses = [item["status"] for item in expected["findings"].values()]

        self.assertEqual(expected["summary"]["checks_completed"], len(statuses))
        self.assertEqual(expected["summary"]["matches"], statuses.count("MATCH"))
        self.assertEqual(expected["summary"]["discrepancies"], statuses.count("DISCREPANCY"))
        self.assertEqual(expected["summary"]["cannot_verify"], statuses.count("CANNOT_VERIFY"))
