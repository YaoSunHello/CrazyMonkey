...
class BeaconFacadeTests(unittest.TestCase):
    ...
    def test_mapper_preserves_large_exact_cent_amount_as_decimal_text(self):
        raw = self.snapshot.model_dump(mode="json")
        raw["findings"][0]["reported_value"] = "90071992547409.01"
        snapshot = ReviewSnapshot.model_validate(raw)

        finding = beacon.to_beacon(snapshot, analyst_mode="DEMO_FIXTURE")["findings"][0]

        self.assertEqual(finding["administratorValue"]["amount"], "90071992547409.01")
        self.assertEqual(Decimal(finding["administratorValue"]["amount"]), Decimal("90071992547409.01"))
    ...
    def test_human_review_does_not_change_computational_status(self):
        ...
        self.assertEqual([item["version"] for item in response.json()["versions"]], [1, 2])

    def test_supporting_upload_preserves_synthetic_provenance(self):
        response = self.client.post(
            "/api/v1/reviews/test-beacon-run/documents",
            data={"role": "SUPPORTING"},
            files={"file": ("additional_support.csv", b"note,value\nsource,1\n", "text/csv")},
        )

        self.assertEqual(response.status_code, 201, response.text)
        self.assertTrue(self.store.calls[-1][3])
